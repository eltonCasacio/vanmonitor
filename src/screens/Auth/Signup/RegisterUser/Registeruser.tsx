...
export const RegisterUser: React.FC = () => {
    ...
    const goToRegisterAddress = async () => {
        if (await validateUser()) navigate('RegisterAddress', user)
    }

    const changeUser = (k: string, v: string) => setUser({ ...user, [k]: v })
    const changeCPF = (cpf: string) => FormatCPF(cpf).then((res) => setUser({ ...user, cpf: res }))
